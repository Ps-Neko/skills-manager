...
test('loadUser: 구버전/수동편집 파일의 name·label·capability 제어문자를 읽기 시 정화(null 보존)', () => {
  const file = tmpFile();
  fs.mkdirSync(path.dirname(file), { recursive: true });
  // 쓰기 검증을 우회해 직접 기록 = 이 fix 이전에 저장됐거나 손으로 고친 파일을 모사.
  fs.writeFileSync(file, JSON.stringify({ version: 1, workflows: [{
    name: 'nm' + ESC2 + '[31mEVIL',
    label: 'Lbl' + ESC2 + '[2K',
    steps: [
      { capability: 'evilcap' + ESC2 + '[2K', skill: null, note: 'n' + ESC2 },
      { capability: 'review', skill: 'user:x' + ESC2 + '[1m' },
    ],
  }] }));
  const [w] = loadUser(file);
  assert.ok(!w.name.includes(ESC2), 'name 제어문자 제거');
  assert.ok(!w.label.includes(ESC2), 'label 제어문자 제거');
  assert.ok(!w.steps[0].capability.includes(ESC2), 'capability 제어문자 제거');
  assert.ok(!w.steps[0].note.includes(ESC2), 'note 제어문자 제거');
  assert.ok(!w.steps[1].skill.includes(ESC2), 'skill 제어문자 제거');
  assert.strictEqual(w.steps[0].skill, null, 'null skill 은 그대로(핀 없음 보존)');
});
