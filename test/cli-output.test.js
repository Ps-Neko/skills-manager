...
test('--menu: 스킬 미설치(빈 HOME)에서도 시작 메뉴를 출력하고 exit 0', () => {
  // 스킬 폴더가 없는 빈 HOME — --menu 핸들러가 인벤토리-부재 조기종료보다 앞에 있어야만 메뉴가 뜬다.
  // execFileSync 는 종료코드 != 0 이면 throw 하므로, 통과 자체가 exit 0 을 보장한다.
  const emptyHome = fs.mkdtempSync(path.join(os.tmpdir(), 'sm-empty-'));
  const out = execFileSync(process.execPath, [SCAN, '--menu'], {
    input: '', encoding: 'utf8',
    env: { ...process.env, HOME: emptyHome, USERPROFILE: emptyHome },
  });
  assert.match(out, /Skills Manager — 시작 메뉴/);
  assert.match(out, /겹치는 스킬 찾기/);
  assert.match(out, /스킬 정리/);
  assert.match(out, /종료/);
});

test('--help workflow: 워크플로우 안내 화면을 출력한다', () => {
  const out = execFileSync(process.execPath, [SCAN, '--help', 'workflow'], { input: '', encoding: 'utf8' });
  assert.match(out, /내 워크플로우/);
  assert.match(out, /단계 스킬 바꾸기/);
  assert.match(out, /뒤로/);
});

test('--help manage: 정리 안내 화면(제거 안전 문구)을 출력한다 — 인자→모드 배선 + 안전 문구 동시 검증', () => {
  const out = execFileSync(process.execPath, [SCAN, '--help', 'manage'], { input: '', encoding: 'utf8' });
  assert.match(out, /스킬 정리/);
  assert.match(out, /휴지통/);
  assert.match(out, /영구삭제 아님/);
});

test('--help (모드 없음): 시작 메뉴로 폴백', () => {
  const out = execFileSync(process.execPath, [SCAN, '--help'], { input: '', encoding: 'utf8' });
  assert.match(out, /Skills Manager — 시작 메뉴/);
});
