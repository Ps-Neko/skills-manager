...
⚠️ **검사·추천·실행 안내는 읽기 전용 — 스킬을 끄거나 지우지 않는다.** 쓰기는 워크플로우 저장 파일(`~/.claude/skillsweep-workflows.json`) 한 곳뿐. **스킬 끄기 기능은 없다** — 겹친 스킬 대부분이 플러그인 안에 있고 플러그인은 통째로만 꺼지는 구조적 벽이라(개인 도구로 영구 미구현).
...
- 진짜 중복 family 목록(루브릭으로 거른 것) + 무리별 "작업 땐 이 중 하나만 쓰면 돼요" + 일상어 이유.
- 생태계 단위 통찰(위 6) + 플러그인 통째-토글 트레이드오프(위 7) — **'끄기'는 사실상 안 된다는 점을 솔직히**.
- 끝에: "보여주는 지도까지만이에요. '끄기'는 플러그인 벽 때문에 없고, 대신 `workflow save`로 흐름마다 하나씩 골라 고정해 쓰세요."
