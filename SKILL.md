...
2. **standalone 스킬이면**:
   a. `node manage-scan.js --remove <이름>` 실행(확인 없음 = **dry-run**) → 무엇이 휴지통으로 갈지 + 함께 정리할 잔여물(워크플로우 핀 등) + **확인 토큰**을 미리 보여준다. **이 토큰은 미리보기를 실제로 돌려야 나온다** — 대상 경로·영향 내용에서 파생된 값이지 스킬 이름이 아니므로, 미리보기를 건너뛰고 지어낼 수 없다.
