...
**[중요] 검사·추천·실행 안내는 읽기 전용 — 스킬을 끄거나 지우지 않는다.** 쓰기는 워크플로우 저장 파일(`~/.claude/skillsweep-workflows.json`) 한 곳뿐. **스킬 끄기 기능은 없다** — 겹친 스킬 대부분이 플러그인 안에 있고 플러그인은 통째로만 꺼지는 구조적 벽이라(개인 도구로 영구 미구현).
...
판정 결과를 데이브(비개발자)가 바로 읽게 찍는다. **반드시 지킬 규칙(어기면 안 됨)**:
- **영어 스킬 이름 금지.** `test-driven-development`·`brainstorming`·`gstack:review` 같은 이름을 쓰지 말고, 무슨 일인지 한국어로("테스트 먼저 짜기"·"아이디어 짜기"·"코드 검토").
- **숫자·전문어 금지.** "8/8 영역"·"capability"·"출처"·"duplicateLevel" 같은 말 빼고 "거의 다"·"여기저기"·"묶음"으로.
- **한 줄 결론 먼저.** 맨 위에 "끌 필요 없고, 자주 하는 작업을 흐름으로 저장하면 끝" 같은 한 줄.
- **이모지·아이콘 금지.**
- **역할 다른 것 가려주기.** 키워드로 묶였어도 일이 다르면(예: 아이디어 6곳 중 '캐묻기'·'상담'은 다른 일) 빼고 "진짜 겹친 건 N곳"으로.
- **끝에 행동 유도.** 끄기 대신 "자주 하는 작업을 '내 흐름'으로 저장" + 예시 명령 한 줄(`/skillsweep workflow save 내흐름`).

좋은 예시 — 이 틀을 따라라:

> **한눈에**: 스킬이 124개 깔렸고, 같은 일이 8가지 겹쳐 있어요. 끌 필요 없고, 흐름 저장으로 정리하면 끝.
> **겹친 일들**: 테스트 먼저 짜기 — 3곳 / 오류 찾기 — 4곳 / 계획 세우기·스펙 작성·보안 점검 — 각 2곳 / 배포 — 3곳 / 아이디어 짜기 — 진짜 2곳(캐묻기·상담은 다른 일이라 뺌) / 코드 리뷰 — 진짜 2곳(리뷰 받기·요청은 다른 일이라 뺌)
> **그래서?**: 끄는 건 구조상 거의 안 돼요(묶음이 통째로만 꺼져서 멀쩡한 것까지 잃음). 대신 자주 하는 작업을 `/skillsweep workflow save 내흐름`으로 저장해 쓰세요.
...
- 쓰기는 워크플로우 저장 파일(`~/.claude/skillsweep-workflows.json`) 한 곳뿐.
