...
**막대 그래프 형식 (3단계 기본).** 곳 수 많은 순, 묶음당 한 줄. 막대 한 칸=1곳, ■=같은 일 수만큼·나머지는 ·. **막대는 줄 끝에 둔다** — 막대 길이가 곧 곳 수라 묶음마다 다르고, ■·은 폭이 들쭉날쭉(한글·기호 폭 문제)이라 가운데 막대로는 세로 정렬이 불가하다. 그래서 앞쪽 라벨·숫자 칸만 **고정폭으로 맞춰 세로 정렬**하고(라벨 12칸·숫자 15칸, 한글=2칸으로 계산해 공백 채움), 막대는 맨 끝에서 자유롭게 뻗게 둔다(끝에 있어 길이가 달라도 정렬이 안 깨짐). 이 형식에 한해 두 규칙을 푼다 — (1) '세로 재나열 금지'를 풀어 묶음당 한 줄씩, (2) '장식 기호 금지'를 풀어 막대 기호 ■·만 허용(그 외 이모지·아이콘은 여전히 금지). **인라인 주석 금지** — 색칠(■)이 막대 전체보다 짧은 묶음(곳 수 > 같은 일)만 아래 '막대가 색칠보다 긴 셋' 블록에서 한 줄로 푼다(줄 안에 주석을 끼우면 정렬이 깨짐). 나머지 규칙(영어 스킬명 금지·숫자 디지털·기계 결론 본문 반복 금지·역할 다른 건 '같은 일'로 보정)은 그대로. 아래는 모양 본보기:
...
막대가 색칠보다 긴 셋 (곳 수는 많아도 진짜 겹치는 건 적음):
 아이디어  — 펼치는 건 3개(범용·신규제품·기존제품). 나머지 5개는 딴 일: 의도 캐묻기·다듬기·실험설계2·OKR.
 코드 리뷰 — '리뷰 받고 반영하기'는 딴 일이라 빠짐.
 배포/출시 — '출시 점검'과 '출시 문서'로 갈려 사실상 안 겹침.

다음 한 수: 자주 쓰는 순서를 '내 흐름'으로 저장. 끌 건 없음.
```

- 위 숫자는 본보기다 — 매번 그날 scan(`--judge`) 결과로 바꿔 채운다.
- 라벨 칸 12·숫자 칸 15는 한글 폭 2칸 기준 고정폭이다(짧으면 공백으로 채워 │·막대 시작점을 줄마다 맞춘다). 막대는 끝이라 정렬 대상 아님.
- '막대가 색칠보다 긴 셋' 블록엔 곳 수 > 같은 일인 묶음만 넣는다(다 같으면 생략). 라벨은 9칸으로 맞춰 ' — ' 위치를 정렬.
